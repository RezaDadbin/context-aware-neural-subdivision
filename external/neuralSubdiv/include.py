...
sys.path.append(module_dir + '/pythongptoolbox/')
sys.path.append(module_dir + '/torchgptoolbox_nosparse/')
    ...
    def getFlapPool(self, HF):
        ...
        for ii in range(nM):
            ...
            for jj in range(nS):
                ...
                poolMat = torch.sparse_coo_tensor(
                    I, val, torch.Size([nV, hfIdx.size(0)]))
        ...
        for ii in range(self.nM):
            ...
            for jj in range(self.nS - 1):
                ...
                if torch.device(device).type == 'mps':
                    self.poolMats[ii][jj] = self.hfList[ii][jj][:, 0].contiguous()
                else:
