...
class SubdNet(torch.nn.Module):
    ...
    def flapNormalization(self, hf, normalizeFeature = False):
        ...
        vec1 = V[:,F[:,1],:] - V[:,F[:,0],:]
        vec2 = V[:,F[:,2],:] - V[:,F[:,0],:]
        FN = torch.cross(vec1, vec2, dim=2) # nF x 2 x 3
    ...
    def oneRingPool(self, fhe, poolMat, dof):
        ...
        '''
        if poolMat.layout == torch.strided:
            if poolMat.dim() != 1 or poolMat.numel() != fhe.size(0):
                raise ValueError('indexed pooling expects one target per half flap')
            fv = fhe.new_zeros((dof.numel(), fhe.size(1)))
            fv.index_add_(0, poolMat, fhe)
        else:
