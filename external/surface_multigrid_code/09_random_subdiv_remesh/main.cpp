...
void normalize_unit_box(Eigen::MatrixXd & V)
{
	...
	double maxCoord = V.maxCoeff();
	if (!std::isfinite(maxCoord) || maxCoord <= std::numeric_limits<double>::epsilon())
	{
		throw std::runtime_error("cannot normalize mesh with zero or invalid bounding box");
	}
	V /= maxCoord;
}
...
int main(int argc, char *argv[])
{
	...
	normalize_unit_box(VO);
	cout << "normalized mesh to unit box before remeshing" << endl;
